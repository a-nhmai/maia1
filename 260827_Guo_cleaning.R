...
#Data: loading--------------------------------
guo_rd <- read.delim("~/maia1/Data/Trait/r_Guo_et_al_2022.txt", sep = ",") |> select(-X)
selected_FIA <- REF_SPECIES <- read.csv("Data/FIA/c_REF_SPECIES.csv") |> select(SCIENTIFIC_NAME)
...
#units from these are likely TRY units since they are imputed from there

write.csv(guo_cd, "~/maia1/Data/Trait/c_Guo_et_al_2022", row.names = FALSE)
